...
long classicMultiplication(long factor_1, long factor_2) {
    ...
    if (factor_1 == 0 || factor_2 == 0) {
        return 0; // With previous implementation of DIGITS() when 0 was given
                  // we had troubles (log(0) doesn't exists)
    }
...
long divideMultiplication(long factor_1, long factor_2) {
    int digits_1 = DIGITS(factor_1), digits_2 = DIGITS(factor_2),
    ...
    return divideMultiplication(a, c) + divideMultiplication(a, d) +
           divideMultiplication(b, c) + divideMultiplication(b, d);
}
