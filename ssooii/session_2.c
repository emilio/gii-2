...
void send_message(message_queue_t queue, message_type_t type, char* content) {
	...
	ENSURE_SUCCESS(message_send(queue, &message, sizeof(message_t) - sizeof(long)));
	printf("... Success!\n");
}
...
void receive_message(message_queue_t queue, message_type_t type) {
	...
	ENSURE_EQUALS(message_receive(queue, &message, sizeof(message_t) - sizeof(long), type), sizeof(message_t) - sizeof(long));
