...
#define LOG(msg, ...)                                                          \
    do {                                                                       \
        ...
        if (data &&                                                            \
            WaitForSingleObject(data->log_mutex, INFINITE) == WAIT_OBJECT_0) { \
...
bool init_lib() {
    ...
    lib.new_shooter =
        (int (*)(char))GetProcAddress(dlllib, "PIST_nuevoPistolero");
    ...
    if (lib.init == NULL || lib.new_shooter == NULL || lib.victim == NULL ||
        lib.shoot == NULL || lib.die == NULL || lib.deinit == NULL ||
        lib.error == NULL)
        return false;
...
int main(int argc, char** argv) {
    size_t i = 0, count = 0, speed = 0, seed = 0;
    GameData* data;
    int ret;
    ...
    for (i = 0; i < count; ++i) {
        ...
        data->threads[i] =
            CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)child_proc,
                         data->statuses + i, 0, NULL);
        if (!data->threads[i])
