...
int semaphore_change_value(semaphore_t sem, unsigned short index, short value) {
    ...
    if (ret == -1)
        FATAL_ERROR("semaphore_change_value (%d, %d, %d): %s", sem, index,
...
void __dump() {
    ...
    for (i = 0; i < data->process_count; ++i)
        fprintf(stderr, "{ %ld, %d}\t", (long)data->children[i].id,
                data->children[i].status);
    fprintf(stderr, "\n");
}
...
void kill_all() {
    ...
    for (i = 0; i < data->process_count; ++i)
        if (data->children[i]
                .id /** If we kill uninitialized memory we may kill ourselves
                       with 0 */
            &&
            kill(data->children[i].id, SIGKILL) == -1)
            LOG("Kill to %ld failed: %s", (long)data->children[i].id,
                strerror(errno));
}
...
int child_proc(char lib_id) {
    ...
    while (1) {
        ...
        /** If I am, give pass to everyone two times: one for start and one for
         * end */
        if (im_coordinator) {
            ...
            /** We cache this value locally because it may change after the
             * W0(S1) */
            ...
            semaphore_change_value(data->semaphores, SEMAPHORE_ALL_SHOOTED,
        ...
        if (msgsnd(data->message_queue, &msg, MESSAGE_MAX_CONTENT_SIZE, 0) ==
            -1)
        ...
        if (im_coordinator) {
            semaphore_change_value(data->semaphores, SEMAPHORE_ALL_RECEIVED,
                                   this_round_alive_count);
        }
        ...
        // empty the queue.
        if (msgrcv(data->message_queue, &message, MESSAGE_MAX_CONTENT_SIZE,
                   lib_id, IPC_NOWAIT | MSG_NOERROR) != -1) {
...
int main(int argc, char** argv) {
    size_t i = 0, count = DEFAULT_PROC_COUNT, speed = DEFAULT_SPEED, seed = 0;
    pid_t current_pid, last_dead_process;
    GameData* data;
    int ret;
    char lib_id = 'A';
