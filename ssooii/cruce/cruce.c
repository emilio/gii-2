 ...
 *   - SEMAPHORE_READY is used at the beginning and is 0 when all processes are
 *ready
 *        We can't use this semaphore later, since one process can lock it, and
 *without
...
/** Process struct */
typedef struct Process { pid_t id; /** PID */ } Process;
...
GameData* manage_data(DataActionType action, size_t count) {
    ...
    if (action == DATA_CREATE && count != 0 && data == NULL) {
        ...
        data = (GameData*)mmap(NULL, size, PROT_READ | PROT_WRITE,
...
int child_proc_walker(size_t my_index) {
    ...
    LOG("Walker current: (%d, %d), next: (%d, %d)", current.x, current.y,
        next.x, next.y);
    ...
    while (next.y >= 0) {
        ...
        if (!GRID_VALUE_FOR(next)) {
            LOG("Moving from (%d, %d) to (%d, %d)", current.x, current.y,
                next.x, next.y);
...
int child_proc_car(size_t my_index) {
    ...
    while (next.y >= 0) {
        ...
        if (!GRID_VALUE_FOR(next)) {
            LOG("Car moving from (%d, %d) to (%d, %d)", current.x, current.y,
                next.x, next.y);
            GRID_VALUE_FOR(current) = 0;
            GRID_VALUE_FOR(next) = 1;
...
#define INIT_RANGE(sem, p1x, p1y, p2x, p2y)                                    \
    do {                                                                       \
...
int main(int argc, char** argv) {
    size_t i = 0, max_count = DEFAULT_PROC_COUNT, speed = DEFAULT_SPEED;
    pid_t current_pid;
    GameData* data;
    int ret, type;
    ...
    /** Another for us */
    data->semaphores =
        semget(IPC_PRIVATE, TOTAL_SEMAPHORE_COUNT, IPC_CREAT | 0600);

    if (data->semaphores == -1)
        FATAL_ERROR("Semaphore creation failed: %s\n", strerror(errno));

    ret = CRUCE_inicio(speed, max_count + 2, data->semaphores,
                       data->library_data);
