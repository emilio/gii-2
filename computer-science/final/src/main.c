...
node_t* atom_new(const char* const val) { return node_new(val, 0); }

node_t* node_nil() { return atom_new("<nil>"); }

node_t* node_append(node_t* node, int count, ...) {
    va_list args;
    int i;
    node = (node_t*)realloc(
        node, sizeof(node_t) + (node->element_count + count) * sizeof(node_t*));
