...
size_t hash(const char* dni) {
    return atoi(dni) % BUCKETS;
}

size_t hash_alumn(alumn_t* alumn) {
    return hash(alumn->dni);
}

void copy_alumn(alumn_t* src, alumn_t* dest) {
    *dest = *src;
}
...
int hash_truncate_file(const char* filename) {
    ...
    DEBUG("Truncating file `%s`. Buckets [%d; %zu]", filename, BUCKETS + OVERFLOW_BUCKETS, sizeof(bucket));
...
int hash_insert(FILE* fp, alumn_t* alumn) {
    ...
    DEBUG("Inserting alumn with dni %s in bucket %zu (%d elements now)", alumn->dni, bucket_index, bucket.element_count);
    ...
    DEBUG("...Searching in overflow area adter increasing bucket %zu element count to %d", bucket_index, bucket.element_count);
    /// Go to overflow zone and search sequentially
    i = 0;
    while ( i < OVERFLOW_BUCKETS) {
        hash_read_bucket(fp, BUCKETS + i, &bucket);

        DEBUG("....Searching in bucket (%zu), %d elements", BUCKETS + i, bucket.element_count);
        /// TODO remove repetition here
        if ( bucket.element_count < REGS_PER_BUCKET ) {
            copy_alumn(alumn, &bucket.alumns[bucket.element_count++]);
            DEBUG("......adding %s to overflow area (bucket %zu)", bucket.alumns[bucket.element_count - 1].dni, BUCKETS + i);
...
int file_from_sequential_to_hash(const char* sequential_file, const char* hash_file) {
    FILE* sequential_fp;
    FILE* hash_fp;
    alumn_t alumn;
...
int main() {
    ...
    for ( i = 0; i < STATIC_ARRAY_LENGTH(tests); ++i ) {
        ...
        printf(" [%c] %s\n", ret == 0 ? 'x': ' ', tests[i]);
        if ( ret == 0 )
            printf("   %s %s %s\n", alumn.first_name, alumn.mid_name, alumn.last_name);
    }
