...
void disjoint_sets_grow(disjoint_sets_t* self) {
    ...
    if (self->sets == NULL) {
        self->sets =
            (disjoint_set_t*)malloc(target_capacity * sizeof(disjoint_set_t));
    } else {
        self->sets = (disjoint_set_t*)realloc(
            self->sets, target_capacity * sizeof(disjoint_set_t));
    }
...
disjoint_set_t disjoint_sets_root_of(disjoint_sets_t* self,
                                     disjoint_set_t set) {
...
int disjoint_sets_are_same_set(disjoint_sets_t* self, disjoint_set_t set_1,
    ...
    return disjoint_sets_root_of(self, set_1) ==
           disjoint_sets_root_of(self, set_2);
}
...
void disjoint_sets_join(disjoint_sets_t* self, disjoint_set_t set_1,
     ...
     * issue.
     */
    self->sets[disjoint_sets_root_of(self, set_1)] =
        disjoint_sets_root_of(self, set_2);
}
