...
void stack_push_(stack_t* stack, char* val) {
    ...
    memcpy(stack->data + (stack->size++ * stack->item_size), val,
           stack->item_size);
}

/** We define a void pop() operation... it's perfectly reasonable using top +
 * pop */
...
void stack_top(stack_t* stack, void* val) {
    assert(val != NULL);
    assert(stack->size != 0);
    memcpy(val, stack->data + ((stack->size - 1) * stack->item_size),
           stack->item_size);
}
