...
void graph_debug_path(graph_t* graph, vertex_id_t initial,
    ...
    printf("Vertex %zu was reached from %zu (distance: %zu)\n", vertex_id,
           reached_from,
           graph->v[vertex_id]->distance - graph->v[reached_from]->distance);
...
void graph_debug_path_info(graph_t* graph, vertex_id_t initial_vertex_id) {
    ...
    for (i = 0; i < graph->size; ++i) {
        if (i == initial_vertex_id) {
            printf("Vertex %zu is the initial vertex, so it has a distance of "
                   "%zu\n",
