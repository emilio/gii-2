...
void b_heap_grow(b_heap_t* heap) {
    size_t target_capacity =
        heap->capacity == 0 ? BINARY_HEAP_INITIAL_CAPACITY : heap->capacity * 2;

    heap->elements =
        realloc(heap->elements, target_capacity * sizeof(b_heap_tuple_t));
...
void b_heap_insert(b_heap_t* heap, b_heap_value_t value,
...
void b_heap_heapify_from(b_heap_t* heap, size_t i) {
    ...
    while (i < heap->size) {
        ...
        /** There are two children, pick the one with less priority and bubble
         * it up */
        if (child_index + 1 < heap->size)
            if (elements[child_index].priority >
                elements[child_index + 1].priority)
                child_index++;
...
void b_heap_set_priority(b_heap_t* heap, size_t index,
                         b_heap_priority_t new_priority) {
...
void b_heap_change_priority(b_heap_t* heap, size_t index,
                            b_heap_priority_t delta) {
    return b_heap_set_priority(heap, index,
                               b_heap_get_priority(heap, index) + delta);
}

/** Note before using this we must store heap.size! */
b_heap_value_t* b_heap_sort(b_heap_t* heap) {
    b_heap_value_t* ret =
        (b_heap_value_t*)malloc(b_heap_size(heap) * sizeof(b_heap_value_t));
    size_t i = 0;
...
/** Return the ordered heap: We extract as before, but with priority, and later
 * reinsert */
void b_heap_heapsorted(b_heap_t* heap) {
    b_heap_tuple_t* temp =
        (b_heap_tuple_t*)malloc(b_heap_size(heap) * sizeof(b_heap_tuple_t));
    size_t size = b_heap_size(heap), i = 0;
