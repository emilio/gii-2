...
void set_grow(set_t* set) {
	set->capacity = set->capacity == 0 ? SET_INITIAL_CAPACITY : set->capacity * 2;

	if ( set->elements == NULL )
		set->elements = (set_element_t*) malloc(set->capacity * sizeof(set_element_t));
	else
		set->elements = realloc(set->elements, set->capacity * sizeof(set_element_t));
}
...
set_t* set_product(set_t* set_1, set_t* set_2) {
	...
	for ( i = 0; i < set_1->size; ++i )
		for ( j = 0; j < set_2->size; ++j )
			set_add(ret, str_concat(set_1->elements[i], set_2->elements[j], ret->max_element_length));
...
int main(int argc, char** argv) {
	size_t size = DEFAULT_MAX_LENGTH,
		   i;
